...
const PopularProducts = () => {
  return (
    <div className="relative mt-[14rem] mx-[1.5rem] w-[390px] h-[573px] md:w-[1440px] md:h-[744px] bg-white">
      ...
      <div 
      className='relative hidden md:bottom-[8rem] md:w-[1440px] md:h-[375px] md:flex gap-[30px] group'>
      <div key="/products/id10" className="hidden md:flex flex-col w-[630px] h-[375px] gap-[2rem]">
       <Image
          src={`/images/sofa.png`}
          alt="Sofa Image"
          width={200}
        ...
        <div className='flex flex-col gap-[8px]'>
            <h4 className='font-clash font-normal leading-7 text-darkPrimary md:text-xl'>The Poplar suede sofa</h4>
            <p className='font-satoshi font-normal leading-[27px] text-darkPrimary md:text-xl'>&pound; 980</p>
        </div>
        
        {/* Add to Cart button */}
        <button 
          className='w-[160px] md:w-[635px] absolute bottom-0 right-[30.5rem] transform -translate-x-1/2 px-4 md:px-8 py-2 md:py-4 bg-black/70 text-white rounded-md opacity-0 group-hover:opacity-100 transition-opacity z-10'>
          <Link href={`/products/id10`} className='text-sm md:text-xl'>Add to Cart</Link>
        </button>
       </div>
        <ProductCard image="BlackChair" heading="The Dandy Chair" price="250" id="4"/>
        <ProductCard image="OldChair" heading="The Dandy Chair" price="250" id="9"/>
      </div>
